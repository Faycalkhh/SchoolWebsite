...
type TopStudent = { rank: 1 | 2 | 3; name: string; level: string; photo: string | null };

const achievements: Record<string, { ar: string; fr: string }> = {
  Hifz:          { ar: "يحفظ القرآن الكريم كاملاً",       fr: "En voie de mémorisation du Coran" },
  Avancé:        { ar: "مستوى متقدم في التجويد والتلاوة",  fr: "Niveau avancé en Tajweed" },
  Intermédiaire: { ar: "تقدم منتظم في حفظ القرآن",         fr: "Progression régulière en mémorisation" },
  Débutant:      { ar: "يسير بثبات في رحلة الحفظ",         fr: "Débuts prometteurs en récitation" },
};

const podiumColors: Record<number, { block: string; border: string }> = {
  1: { block: "bg-[#c9a84c]", border: "border-[#c9a84c]" },
  2: { block: "bg-[#2d6a4f]", border: "border-[#52b788]" },
  3: { block: "bg-[#1a3a2c]", border: "border-[#2d6a4f]" },
};
...
function Avatar({ name, photo, size }: { name: string; photo: string | null; size: string }) {
  if (photo) {
    ...
    return <img src={src} alt={name} className={`${size} rounded-full object-cover w-full h-full`} />;
  }
  const initials = name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase();
  return (
    <div className={`${size} w-full h-full rounded-full bg-[#2d6a4f] flex items-center justify-center`}>
...
export default function TopStudents() {
  ...
  const podiumOrder = [
    students.find((s) => s.rank === 2),
    students.find((s) => s.rank === 1),
    students.find((s) => s.rank === 3),
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 lg:px-8">
          ...
          {podiumOrder.length === 0 ? (
            ...
            <div className="flex items-end justify-center gap-2 sm:gap-4 md:gap-8">
              {podiumOrder.map((student) => {
                ...
                const achievement = achievements[student.level]?.[lang] ?? "";
                ...
                return (
                  <div key={student.rank} className="flex flex-col items-center w-[30%] max-w-[120px] sm:max-w-[140px] md:max-w-[192px]">
                    <div className="text-center mb-2 sm:mb-3 px-1 w-full">
                      ...
                      >
                        <Avatar name={student.name} photo={student.photo} size="" />
                      </div>
                    ...
                    <div className={`w-full ${blockH} ${colors.block} rounded-t-xl flex flex-col items-center justify-center gap-1`}>
                      <span className="text-white font-black text-xl sm:text-2xl md:text-3xl">{student.rank}</span>
                      {isFirst && <span className="text-white/70 text-[10px] tracking-widest">🥇</span>}
                    </div>
                  </div>
                );
              })}
        ...
        <AnimateIn delay={0.3}>
          <div className="mt-14 text-center">
            <p className="text-[#c9a84c]/80 text-lg italic">{T.students.quote}</p>
